...
- **Success envelopes** come from `shared/src/api.ts`: `Single<T>` is `{ message, data }`, `Paginated<T>` is `{ message, data, meta }` (see [Pagination](#pagination)). They mirror the error shape: `message` is user-presentable with no internals, and the client never branches on it — it branches on the HTTP status and reads `data`. The status lives only in the HTTP status line, never repeated in the body. `204` responses have no body.
...
|---|---|---|---|---|---|---|
| `GET` | `/api/campgrounds` | public | — | — | `200` `{ message, data: [Campground], meta }` | `400` bad page param |
| `POST` | `/api/campgrounds` | token | isLoggedIn → validateCampground | `campgroundBody` | `201` `{ message, data: Campground }` | `400` `401` |
| `GET` | `/api/campgrounds/:id` | public | — | — | `200` `{ message, data: Campground }` — author and reviews populated | `404` |
| `PATCH` | `/api/campgrounds/:id` | owner | isLoggedIn → isAuthor → validateCampground | `campgroundPatch` | `200` `{ message, data: Campground }` | `400` `401` `403` `404` |
...
| Method | Path | Auth | Middleware | Request body | Success | Errors |
|---|---|---|---|---|---|---|
| `GET` | `/api/campgrounds/:id/reviews` | public | — | — | `200` `{ message, data: [Review], meta }` | `404` campground |
| `POST` | `/api/campgrounds/:id/reviews` | token | isLoggedIn → validateReview | `reviewBody` | `201` `{ message, data: Review }` | `400` `401` `404` |
| `GET` | `/api/campgrounds/:id/reviews/:reviewId` | public | — | — | `200` `{ message, data: Review }` | `404` |
| `PATCH` | `/api/campgrounds/:id/reviews/:reviewId` | owner | isLoggedIn → isReviewAuthor → validateReview | `reviewPatch` | `200` `{ message, data: Review }` | `400` `401` `403` `404` |
...
|---|---|---|---|---|---|
| `POST` | `/api/auth/register` | public | `{ email, displayName, password }` | `201` `{ message, data: User }` + access token + refresh cookie | `400` `409` email taken |
| `POST` | `/api/auth/login` | public | `{ email, password }` | `200` `{ message, data: User }` + access token + refresh cookie | `400`, `401` generic |
| `POST` | `/api/auth/refresh` | refresh cookie | — | `200` `{ message, data: { accessToken } }` + new refresh cookie | `401` missing/expired/revoked |
...
| `GET` | `/api/me` | token | — | `200` `{ message, data: User }` | `401` |
| `POST` | `/api/auth/verify-email/request` | token | — | `204` — sends a verification link | `401`, `429` |
| `POST` | `/api/auth/verify-email` | public | `{ token }` | `200` `{ message, data: User }` | `400` invalid/expired token |
...
| Method | Path | Auth | Request body | Success | Errors |
|---|---|---|---|---|---|
| `POST` | `/api/uploads/signature` | token | `{ contentType }` | `200` `{ message, data: { url, fields, key } }` — an S3 presigned POST | `400` unsupported type, `401`, `429` |
...
| Method | Path | Auth | Request body | Success | Errors |
|---|---|---|---|---|---|
| `GET` | `/api/users/:id` | public | — | `200` `{ message, data: PublicUser }` — `displayName`, `avatar_url`, `createdAt` only | `404` |
| `GET` | `/api/users/:id/campgrounds` | public | — | `200` `{ message, data: [Campground], meta }` | `404` |
| `PATCH` | `/api/me` | token | `{ displayName?, avatar_url? }` | `200` `{ message, data: User }` | `400` `401` |
...
|---|---|---|---|---|
| `GET` | `/api/health` | public | `200` `{ message, data: Health }` when the database connection is ready — `Health` is `{ status: 'ok' }` | `503` `UPSTREAM_UNAVAILABLE` otherwise |
...
- **Response envelope**: `{ message, data: [...], meta: { page, limit, total, totalPages } }`. Same envelope everywhere, matching the error contract's single shape.
